...
llattice invariants (the weight-domain semirings, the status algebra) are
...
REGISTRY = ROOT / "proofs" / "doc" / "lattice-invariants.tsv"
...
def main() -> int:
    ...
    if failures.messages:
        ...
        return 1
    print(
        f"check-lattice-invariants: {len(rows)} invariants verified "
