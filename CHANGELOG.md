...
- Comprehensive `docs/` tree: theory (order theory, lattices, lawfulness proofs, the semiring bridge), design
  ...
  catalog.
- 17 fully-coloured, diagrams-as-code figures rendered from committed sources via `make -C docs/diagrams`
  (Graphviz, D2, Mermaid, TikZ), with a shared colour palette and a reproducible render pipeline.
...
- **Corrected the lawfulness claims** in `src/lib.rs` rustdoc and `README.md`. The previous blanket "satisfies
  ...
  - `f32`/`f64` form a lattice (a chain) only on the `NaN`-free extended reals `[−∞, +∞]`; with `NaN`,
    idempotency-under-`==` and the order break.
  ...
  - `HashSet<T>` realises the bounded-below distributive-lattice fragment at runtime (`⊥ = {}`, no `⊤`/complement);
    only the mathematical powerset `𝒫(U)` is a complete atomic Boolean algebra.
- No behavioural or API changes — corrections are to documentation and comments only.

## [0.1.0]

### Added
- The `Lattice` trait — `join` (least upper bound, `⊔`) and `meet` (greatest lower bound, `⊓`) — with the
  supertrait bound `Clone + Send + Sync`.
