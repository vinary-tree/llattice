...
```math
a \preceq b \;:\iff\; a \oplus b = b.
```

Then $`(S, \preceq)`$ is a join-semilattice whose join is $`\oplus`$ and whose least element is $`\bar{0}`$.
...
partial order via $`a \preceq b :\iff a \oplus b = b`$, under which $`\oplus`$ is the least upper bound. The additive identity $`\bar{0}`$
satisfies $`\bar{0} \oplus a = a`$, i.e. $`\bar{0} \preceq a`$ for all $`a`$, so $`\bar{0} = \perp`$. ∎

> **So the bridge is real and one-directional-cheap:** *give me any idempotent semiring and I hand you a
> join-semilattice with `join` $`= \oplus`$.* This is exactly the
> `JoinSemilattice` capability that the `lling-llang` bridge must expose for its
> semiring types.
...
Having identified $`\oplus`$ with $`\vee`$, the seductive next step is to identify $`\otimes`$ with $`\wedge`$. **This is a category
...
- In min-plus, $`a \otimes b = a + b`$ (concatenate edge weights), whereas $`a \wedge b`$ would be $`\max(a, b)`$ — different
...
- $`\otimes`$ need not be commutative (matrix dioids, weighted automata), whereas $`\wedge`$ always is.
...
If a dioid *also* happens to be a lattice (has a genuine $`\wedge`$), that meet is an additional structure, not $`\otimes`$.

---

## 4. Why the bridge lives in `lling-llang`, not here

`llattice` is a **leaf crate**: it owns the layered lattice vocabulary and nothing else (see
...
2. **The orphan rule is respected without contortion.** `lling-llang` defines `IdempotentSemiring`, so it is
   allowed to bridge its owned semiring types to `JoinSemilattice`—exposing
   $`\oplus`$ as `join` and $`\bar{0}`$ as `Bottom`. It must not claim
   `Lattice` without an independent meet and absorption evidence. (See
   [design/02 — the orphan rule](../design/02-orphan-rule.md).)
...
The [semiring bridge figure](figures/semiring-bridge.svg) makes the ownership
and one-way mapping explicit: lling-llang depends on llattice, maps semiring
addition to join and zero to bottom, and makes no meet claim from
multiplication.
...
- Every idempotent semiring has a **natural order** making $`\oplus`$ a `join` and $`\bar{0}`$ a $`\perp`$ (a join-semilattice, for
  free).
...
- Therefore the bridge belongs in the crate that owns the semiring types — `lling-llang` — keeping `llattice` a
  pure, dependency-free leaf and making the $`\otimes`$/$`\wedge`$ confusion unrepresentable here.
